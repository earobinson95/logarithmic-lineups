...
#              target,
#              labeller = label_both) +
#   scale_fill_brewer(palette = "Dark2", direction = -1, label = c("Incorrect", "Correct")) +
...
poo <- lineup_model_data |> 
  group_by(scale, curvature) |> 
  reframe(ntotal = n(),
          ncorrect = sum(binary_response),
          prop_correct = ncorrect/ntotal
          ) |> 
  ungroup() |> 
  arrange(prop_correct) |> 
  group_by(curvature) |>
  reframe(avgprop = mean(prop_correct)) |> 
  arrange(-avgprop)
poo
```
...
picsList <- paste0("../images/", str_remove_all(curvature_order, "-"), "_log.png")
