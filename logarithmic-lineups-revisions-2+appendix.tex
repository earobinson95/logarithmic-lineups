...
{\centering \includegraphics[width=1\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/log-scales-1} 
...
We simulated data based on a two-parameter exponential function with a
vertical shift and multiplicative errors: \begin{align}
y_i & = \alpha\cdot e^{\beta\cdot x_i + \epsilon_i} + \theta \\
\text{with } \epsilon_i & \sim N(0, \sigma^2) \nonumber
\end{align}

where \(\alpha\) controls the vertical stretch, \(\beta\) affects the
rate of growth, and \(\theta\) provides a vertical shift of the
function. The parameters \(\alpha\) and \(\theta\) were adjusted based
on \(\beta\) and \(\sigma^2\) to guarantee the range and domain
constraints are met. It is worth noting that while the inclusion of
\(\theta\) allows for constraints on the range, when a log
transformation is applied, the curve looses the linearity properties
typically seen with log transformed exponential data. The equation
generated \(N = 50\) points \((x_i, y_i), i = 1,...,N\) where \(x\) and
\(y\) have an increasing exponential relationship. We selected
parameters and generated data using a heuristic approach rather than
algebraic to ensure the parameter selections resulted in visually
distinct curves. In summary, we identified three points (start,
midpoint, and end) we wanted the curve to pass through. These control
the level of curvature seen in the data. Then we optimized a nonlinear
least squares regression line using the function from Equation (1) to
obtain parameters used for simulating the final data sets with
multiplicative errors. We provide details for the heuristic data
generation procedure in \cref{alg:lineup-parameter-estimation-algorithm}
and \cref{alg:lineup-exponential-data-simulation-algorithm}.
...
{\centering \includegraphics[width=1\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/lof-density-curves-1} 
...
the experimental lineups, we generated three sets of homogeneous
``Rorschach'' lineups, each featuring panels simulated from the same
distribution. Importantly, participants were unaware that there was no
designated target panel for identification. Each participant evaluated
one ``Rorschach'' lineup.

\begin{figure}[tbp]

{\centering \includegraphics[width=1\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/curvature-combination-example-1} 
...
{\centering \includegraphics[width=\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/lineup-example-1} 
...
most regions; 19+ in certain U.S. states) in their region; we did not
request a representative sample, previous literature suggests there are
minor effects of demographics on the outcome of graphical experiments
involving lineups
...
evaluated was assigned a biary value based on the participant response
...
\hypertarget{rorschach-evaluations}{%
...
larger \(\alpha = 0.4.\) Through a visual examination of ``Rorschach''
evaluations and calculation of \(\alpha\) parameters, we determined that
our null sampling method is appropriate.

\begin{figure}[tbp]

{\centering \includegraphics[width=\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/rorschach-lineups-1} 

}

\caption{Observed proportions of panel selections for the 'Rorschach' lineups. The darker shade (bottom) denotes panels chosen more frequently than others. The annotated text provides the calculated $\alpha$ parameter of the Dirichlet distribution that represents individuals selections for the Rorschach plots.}\label{fig:rorschach-lineups}
\end{figure}

\hypertarget{participant-accuracy}{%
...
curvature null plots). Additionally, accuracy increased when data was
displayed on the log scale compared to the linear scale in all curvature
scenarios with an exception of a medium target curve embedded in low
null curves.
...
found that, in general, participants who correctly identified the target
plot were 0.556 (se 0.039) Likert scale points more confident, on
average, across all conditions (F = 214.57; df = 1, 3709; p \textless{}
0.0001). Similarly, on average, lineups evaluated on the log scale
indicated participants were only 0.075 (se 0.0366) Likert scale points
more confident than lineups evaluated on the linear scale (F = 3.70; df
= 1, 3709; p = 0.054). We discuss further details regarding selection
reasoning and confidence level in the appendix.
...
{\centering \includegraphics[width=\linewidth,]{logarithmic-lineups-revisions-2_files/figure-latex/odds-ratio-plot-1} 
...
{\centering \includegraphics[width=\linewidth,]{appendix-2_files/figure-latex/simple-exponential-1} 
...
{\centering \includegraphics[width=.6\linewidth,]{appendix-2_files/figure-latex/lineup-fixed-scales-1} 
...
{\centering \subfloat[Lineup\label{fig:lineup-free-scales-1}]{\includegraphics[width=.6\linewidth,]{appendix-2_files/figure-latex/lineup-free-scales-1} }\subfloat[Overlay of null and signal panels.\label{fig:lineup-free-scales-2}]{\includegraphics[width=.2\linewidth,]{images/curvature-overlay-free-scales} }
...
{\centering \includegraphics[width=.6\linewidth,]{appendix-2_files/figure-latex/lineup-truncate-scales-1} 
...
{\centering \includegraphics[width=.6\linewidth,]{appendix-2_files/figure-latex/lineup-scale-data-1} 
...
to non-parametric and multinomial methods. Due to the interpretability of
the linear mixed model, we proceed to present the findings from this
analysis. Overall, participants who correctly identified the target plot
were more confident in their plot choice across all conditions (F =
214.57; df = 1, 3709; p \textless{} 0.0001). Overall, on average,
individuals who were correct are 0.556 (se 0.039) Likert scale points
more confident than individuals who were incorrect. We did not find
convincing evidence of a discernable difference in confidence levels
between scales (F = 3.70; df = 1, 3709; p = 0.054). Overall, on average,
lineups evaluated on the log scale indicated participants were 0.075 (se
0.0366) Likert scale points more confident than lineups evaluated on the
linear scale.

\begin{figure}[tbp]

{\centering \includegraphics[width=\linewidth,]{appendix-2_files/figure-latex/conf-mosaic-1} 
