...
Let us consider a simple equation which we could use to simulate data which grows exponentially in $x$, with optional error term $\epsilon$: \begin{align}y &= \exp\left\{\beta_1 x + \epsilon \right\}\quad \text{where}\quad \epsilon \sim N(0, \sigma).\label{eq:simpleexp}\end{align}
...
logplot <- ggplot(df, aes(x = x, y = y, color = b1)) + 
  ...
  scale_color_discrete("Slope", labels = expression(beta[1]==0.06, beta[1]==0.13, beta[1]==0.23)) + 
  theme_bw()  + 
